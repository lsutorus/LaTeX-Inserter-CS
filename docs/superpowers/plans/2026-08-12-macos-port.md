...
| 10 — Permission status UX | ⏳ next session | — | Extends modified `SettingsViewModel`/`SettingsWindow.axaml` (already carry `CurrentVersion` WIP from prior commit). |
...
**Resume state:** Tasks 1–9 committed on `feat/macos-port` (Task 6 code only — Steps 8–9 await a Mac; Task 7 code complete, behavior verified in Task 16; Task 8 code complete, Step 6 calibration awaits a Mac; Task 9 code complete, `+mainAppService` selector and bundle-based register/unregister await a Mac). Working tree clean. Start Task 10 in a fresh session. Run `dotnet test LaTeXInserter.slnx` first to confirm green baseline (148 passing, 2 skipped) before changes.

**Outstanding MAC-ONLY debt:** Task 6 Steps 8–9 (prove libuiohook delivers events under Avalonia's run loop; apply the run-loop fallback if it does not). This is the port's riskiest unverified assumption and gates Tasks 12/14/16. Task 7's activation behavior (frontmost capture, `deactivate` + `activateWithOptions:` restore under macOS 14+ activation restrictions) is likewise unverified — it is covered by Task 16's matrix. Task 8 Step 6 adds two more on-device checks: cursor-coordinate calibration across mixed-scale displays (multiply by `screen.Scaling` only if measured), and confirming whether Avalonia 12.0.4's platform handle descriptor is `NSView` (plan's assumption, needs the `-[NSView window]` hop) or `NSWindow` (use the handle directly in `MacWindowBehavior.ConfigureOverlay`). Task 9 adds a third: confirm `+[SMAppService mainAppService]` is the real selector behind Swift's `SMAppService.mainApp`, and that `registerAndReturnError:` / `unregisterAndReturnError:` succeed from an installed `.app` bundle.
