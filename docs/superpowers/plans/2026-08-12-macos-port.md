...
- [x] **Step 2: Make the tray icon platform-conditional**
...
- [x] **Step 3: Build and smoke-test Windows** (build only — run skipped, needs admin + tray)
...
- [x] **Step 1: Condition the Windows-only properties**
...
- [x] **Step 2: Verify both publishes configure**
...
- [x] **Step 3: Write `build/macos/Info.plist`**
...
- [x] **Step 4: Write `build/macos/entitlements.plist`**
...
- [x] **Step 5: Write `build/macos/make-icns.sh`**
...
| 7 — macOS window activator | ✅ done (code) — **behavior unverifiable on Windows** | `647dcec` | `MacWindowActivator.cs` written verbatim from plan; no deviation needed. All `ObjC` helpers the plan calls (`Send` int overload, `SendInt`, `SendBoolByte`, `SendBoolUlong`, `LoadFramework`, `AppKitPath`) already exist from Task 5 with matching signatures. Build: 0 errors, 14 warnings — all pre-existing (`WindowsStartupRegistrar` CA1416 ×9, `MacInteropTests` CA1416 ×3, xUnit1031, xUnit2013); the class-level `[SupportedOSPlatform("macos")]` suppresses CA1416 on its own ObjC call sites, as the plan predicted. Plan declares no automatable test (activation behavior lands in Task 16's manual matrix), so the count is unchanged: 146 pass, 2 skipped. Note for the Mac session: `-[NSApplication activateIgnoringOtherApps:]` and `-[NSApplication deactivate]` both return `void`, but the plan routes them through `SendBoolByte`/`Send` (IntPtr-returning); harmless under the arm64 ABI (the return register is just garbage and discarded), kept verbatim rather than drifting. Not registered in DI — Task 9 owns that. |
| 8 — macOS overlay positioner | ✅ done (code) — **blocked on hardware for Step 6** | `c471950` | `MacOverlayPositioner.cs` + `MacWindowBehavior.cs` written verbatim from plan; no deviation. Step 2: the negative-origin clamp test **passed on first run** — `Helpers/OverlayPositioner.GetPosition` already clamps against `workingArea.X`/`.Y` (lines 24–29), so the helper is origin-safe and was left untouched (plan's Step 7 `git add` lists it; dropped from the commit since it is unmodified). All `ObjC` helpers used (`Send` IntPtr overload, `SendVoidUlong`) exist from Task 5. Class-level `[SupportedOSPlatform("macos")]` suppresses CA1416 on the new call sites — build warning count unchanged (14, all pre-existing). Step 6 (multi-display coordinate calibration: `ScreenFromPoint`, scale-factor offset, `handle.HandleDescriptor` NSView-vs-NSWindow check in `MacWindowBehavior`) **not executed: requires macOS hardware**. Not registered in DI — Task 9 owns that. 147 tests pass (+1), 2 skipped. |
| 9 — macOS startup registrar + DI branch | ✅ done (code) — **blocked on hardware for selector check** | `9022a46` | `MacStartupRegistrar.cs` written verbatim from plan; macOS DI branch activated in `PlatformServiceRegistration` with the plan's `#pragma warning disable CA1416` around the case block. Test needed `using System.Linq;` **and** `using LaTeXInserter.Platform.MacOS;` (plan named both) — descriptor-only assertions, so no macOS type is instantiated on Windows. All `ObjC` helpers used (`LoadFramework`, `ServiceManagementPath`, `SendBool`, `SendLong`, `Send`, `Sel`, `GetClass`) exist from Task 5 with matching signatures. Build: 0 errors, 14 warnings — identical to baseline, the pragma + class-level `[SupportedOSPlatform("macos13.0")]` add none. **On-device debt:** confirm `+[SMAppService mainAppService]` resolves (plan's fallback: dump `class_copyMethodList` if it returns `nil`), and that register/unregister work from a real `.app` bundle (`dotnet run` loose will fail by design). 148 tests pass (+1), 2 skipped. |
| 10 — Permission status UX | ✅ done (code) — **panel unrenderable on Windows by design** | `1dd7e74` | `SettingsViewModel` gained the 5th ctor param `IPermissionService` (appended, existing order untouched), the 4 `[ObservableProperty]` fields, `RefreshPermissions()`, `BuildSummary`, and the 3 `[RelayCommand]`s — all verbatim from the plan. **Deviation:** the plan's Step 1 assumes `SettingsViewModelTests` already has a `CreateSut` factory; it does not — the 7 existing tests call `new SettingsViewModel(...)` positionally. Added `CreatePermissions()` (substitute with `RequiresUserAction == false`) + a `CreateSut(IPermissionService? = null)` factory for the new tests, and appended `CreatePermissions()` to the 7 existing call sites (they need their own `svc`/`accent`/`startup` handles, so they cannot all route through `CreateSut`). `RefreshPermissions()` called at the end of `Open()`, so the ctor path exercises it too. AXAML panel inserted directly above the `<!-- General Section -->` block per the plan's wording (after the Appearance divider, not at the very top of the window); `RefreshPermissionsCmdCommand` binding name is correct as the plan warned. `AppManager`: `HookFailed += OnHookFailed` wired in `InitializeAsync` next to `HotkeyPressed`, handler added, unsubscribed in `Dispose()`. Build: 0 errors, 14 warnings — identical to baseline, no new CA1416 (nothing here touches platform APIs directly). Step 7's `dotnet run` smoke test **skipped**: needs admin elevation + an interactive tray on Windows; the panel is `IsVisible=false` on Windows anyway (`NoOpPermissionService.RequiresUserAction == false`), which the `PermissionPanel_HiddenWhenPlatformNeedsNoPermissions` test covers. Real panel rendering + the "Open Accessibility…" / "Open Input Monitoring…" deep links land in Task 16's matrix. 152 tests pass (+4), 2 skipped. |
| 11 — macOS menu-bar icon | ✅ done (code) — **glyph unreviewable on Windows** | `b82cf45` | **Deviation on Step 1:** the plan says to derive the asset from `LaTeX-Inserter-icon-final.png`, but that file (and the `.ico`) is a 512×381 *wordmark banner* — white "LaTeX Inserter" lettering on a bar, not a square icon. The only square-ish element is the red σ-in-checkmark mark at source bbox `96,137–180,224` (85×88). Generated `tray-macos.png` programmatically (PowerShell + `System.Drawing`): cropped that mark, masked to reddish pixels only, recolored every non-transparent pixel to flat `#8A8A8A` preserving alpha, bicubic-scaled to 27×28 and centered on a 36×36 transparent canvas (~4 px padding, glyph spans rows/cols 4–31). Verified on disk: 36×36 `Format32bppArgb`, opaque pixels read `#8A8A8A`, corners `A=0`. **Review the glyph by eye** — it is machine-derived, not hand-drawn; replacing the PNG needs no code change. Steps 2 was applied verbatim (`Icon` attribute dropped from `App.axaml`'s `TrayIcon`; `LoadTrayIcon()` + `using Avalonia.Platform;` / `using LaTeXInserter.Models;` added to `App.axaml.cs`). csproj already globs `Assets\**` as `AvaloniaResource`, so no csproj change; confirmed `/Assets/tray-macos.png` is packed into the built dll. Note the XAML `Icon="/Assets/….ico"` path Avalonia previously used is itself `new WindowIcon(AssetLoader.Open(uri))`, so the Windows path is equivalent, not merely similar. Step 3's `dotnet run` smoke test **skipped**: needs admin elevation + an interactive tray. Build: 0 errors, 14 warnings — identical to baseline. Plan declares no automatable test, so the count is unchanged: 152 pass, 2 skipped. Avalonia loads the tray image from a stream and so cannot mark it an NSImage *template* — the icon will not auto-invert between light and dark menu bars; **Task 17 must record this in `docs/architecture.md`**. |
| 12 — Overlay focus + paste timing | ✅ done (code) — **timing values unverified on Windows and Mac** | `2cd0fb0` | `InputSimulatorService` rewritten verbatim from the plan's Step 3 (4-arg test ctor + DI 2-arg ctor, `PasteBlocked`, secure-input early return, `WindowsHoldMs=10`/`MacHoldMs=40`); the old `RuntimeInformation.IsOSPlatform` branch is gone. `IInputSimulatorService` gained the `PasteBlocked` event declaration. `SubmitPasteService` ctor param is now `int? pasteDelayMs = null` defaulting to 120 ms on macOS / 50 ms on Windows (`using LaTeXInserter.Models;` added); no caller passes the argument, so Windows behavior is unchanged. `OverlayWindow.axaml.cs`: `DeactivateGrace` (400 ms) + `_shownAtUtc`, set in the `IsVisibleProperty` branch, guarded in `OnDeactivated` — all verbatim. **Deviation on Step 7:** the plan says "AppManager subscribes" but `AppManager` had no `IInputSimulatorService`; added it as the 7th ctor param (inserted after `submitPasteService`, before `updateCoordinator`, mirroring `Program.cs` DI order — no test constructs `AppManager`, so nothing broke), plus `OnPasteBlocked` and the `Dispose()` unsubscribe. `OnPasteBlocked` posts through `Dispatcher.UIThread` because `SimulatePasteAsync` runs off the paste continuation, unlike `OnHookFailed` whose source already marshals. **Minor plan inconsistency, not a defect:** the task's *Interfaces* header lists the test ctor as `(IEventSimulator, PlatformKind, int)` while its Step 1/Step 3 code uses `(IEventSimulator, IPermissionService, PlatformKind, int)` — followed the code. Step 6's `OverlayViewModelTests` caveat was moot: cancel-on-deactivate lives in the View, and no test asserts it. Step 8's `dotnet run` smoke test **skipped**: needs admin elevation + an interactive tray. Build: 0 errors, 14 warnings — identical to baseline. 155 tests pass (+3), 2 skipped. **On-device debt:** the 40 ms modifier hold, the 120 ms focus-settle delay, and the 400 ms deactivation grace are all starting points — Task 16 must measure whether they are sufficient (too-short grace ⇒ overlay self-dismisses on appear; too-long ⇒ a genuine click-away is ignored). |
| 13 — csproj conditioning + bundle assets | ✅ done | `1de504f` | csproj `PropertyGroup` split verbatim per the plan; `ApplicationManifest` + `ApplicationIcon` now sit in the RID-conditioned group. Verified with `dotnet msbuild -getProperty:ApplicationIcon -getProperty:ApplicationManifest`: both empty for `RuntimeIdentifier=osx-arm64`, both populated for `win-x64` **and** for no RID (the `== ''` arm keeps plain `dotnet build`/`dotnet test` on the Windows path). `Info.plist`, `entitlements.plist`, `make-icns.sh` written verbatim. **Deviation on Step 2:** the plan's `/p:PublishAot=false` fails under Git Bash — MSYS rewrites `/p:…` into a Windows path, yielding `MSBUILD : error MSB1008: Only one project can be specified.`; used `-p:PublishAot=false` instead (same semantics). With that fix `dotnet build -r osx-arm64` **succeeds**: 0 errors, 9 warnings, all the pre-existing `WindowsStartupRegistrar` CA1416 set (the 5 test-project warnings are absent only because Step 2 builds `src/` alone). The `win-x64` publish reproduced the known environmental `MSB3073 … 'vswhere.exe' is not recognized` at the native link step — ILC itself emitted **0** `IL####` warnings, so `publish-win-check/LaTeXInserter.exe` could not be produced and the icon could not be eyeballed; the `-getProperty` check above covers the same ground. `publish-win-check` deleted. `make-icns.sh` committed mode `100755` via `git update-index --chmod=+x` (Windows `core.filemode` would otherwise drop the bit). **Two notes for the Mac sessions.** (1) `make-icns.sh` points at `LaTeX-Inserter-icon-final.png`, which Task 11 established is a **512×381 wordmark banner**, not a 1024px square — `sips -z <n> <n>` forces exact dimensions, so every generated `.icns` slice will be stretched. Cosmetically low-stakes (`LSUIElement` means no Dock icon; the `.icns` shows only in Finder/Get Info), but a square source PNG should replace it before release. (2) `core.autocrlf=true` on this machine: the repo blob is LF and a macOS clone gets LF, but a Windows checkout gets CRLF — Tasks 14 and 15 add more `.sh` files, so a `.gitattributes` carrying `*.sh text eol=lf` is worth adding then (out of Task 13's declared file list, so not added here). Plan declares no automatable test; 155 pass, 2 skipped — unchanged. |
| 14 — Local unsigned macOS build | ⏳ next session [MAC-ONLY] | — | Consider adding `.gitattributes` with `*.sh text eol=lf` alongside `build-local.sh` (see Task 13 note). |
...
**Resume state:** Tasks 1–13 committed on `feat/macos-port` (Task 6 code only — Steps 8–9 await a Mac; Task 7 code complete, behavior verified in Task 16; Task 8 code complete, Step 6 calibration awaits a Mac; Task 9 code complete, `+mainAppService` selector and bundle-based register/unregister await a Mac; Task 10 code complete, panel rendering + Settings deep links await a Mac; Task 11 code complete, the machine-derived `tray-macos.png` glyph wants a human eye and its menu-bar rendering awaits a Mac; Task 12 code complete, its three timing constants await measurement on both platforms; Task 13 code complete, the `.icns` source PNG is the wrong aspect ratio and the `win-x64` native link cannot run in this environment). Working tree clean. **Task 14 is [MAC-ONLY] — it cannot start on Windows.** Run `dotnet test LaTeXInserter.slnx` first to confirm green baseline (155 passing, 2 skipped) before changes.

**Outstanding MAC-ONLY debt:** Task 6 Steps 8–9 (prove libuiohook delivers events under Avalonia's run loop; apply the run-loop fallback if it does not). This is the port's riskiest unverified assumption and gates Tasks 12/14/16. Task 7's activation behavior (frontmost capture, `deactivate` + `activateWithOptions:` restore under macOS 14+ activation restrictions) is likewise unverified — it is covered by Task 16's matrix. Task 8 Step 6 adds two more on-device checks: cursor-coordinate calibration across mixed-scale displays (multiply by `screen.Scaling` only if measured), and confirming whether Avalonia 12.0.4's platform handle descriptor is `NSView` (plan's assumption, needs the `-[NSView window]` hop) or `NSWindow` (use the handle directly in `MacWindowBehavior.ConfigureOverlay`). Task 9 adds a third: confirm `+[SMAppService mainAppService]` is the real selector behind Swift's `SMAppService.mainApp`, and that `registerAndReturnError:` / `unregisterAndReturnError:` succeed from an installed `.app` bundle. Task 11 adds a fifth: confirm the derived `tray-macos.png` is legible as an `NSStatusItem` glyph on both light and dark menu bars (it cannot auto-invert — see the Task 11 note), and that Avalonia decodes it into the status item at all. Task 10 adds a fourth, lower-risk one: confirm the permission panel actually renders and lays out in the Settings window on macOS (it is unconditionally collapsed on Windows), and that the `x-apple.systempreferences:` deep links from `MacPermissionService` open the right System Settings panes. Task 12 adds three timing constants that are guesses until measured: the 40 ms modifier hold (`MacHoldMs`), the 120 ms focus-settle delay in `SubmitPasteService`, and the 400 ms `DeactivateGrace` in `OverlayWindow` — the last is two-sided, since too short lets the overlay self-dismiss as the app comes forward while too long swallows a genuine click-away. A Windows regression pass on the paste path is also owed, since `InputSimulatorService` was rewritten rather than extended.
